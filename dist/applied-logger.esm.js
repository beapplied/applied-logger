...
unwrapExports(PlSqlFormatter_1);
...
unwrapExports(StandardSqlFormatter_1);
...
var getTokens = function getTokens(json, options = {}) {
  let input;
  ...
  do {
    foundToken = false;
    for (let i = 0; i < tokenTypes$1.length; i++) {
      const match = tokenTypes$1[i].regex.exec(input);
      if (match) {
        tokens.push({ type: tokenTypes$1[i].tokenType, value: match[0] });
...
var reIsNative$1 = RegExp('^' +
  funcToString$2.call(hasOwnProperty$5).replace(reRegExpChar$2, '\\$&')
  .replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, '$1.*?') + '$'
);
...
var colorize = function colorize(tokens, options = {}) {
...
exports.constuctError = (err, req) => {
    ...
    return lib(JSON.stringify(errJson), { pretty: true })
};
...
export { debug, error, info, log, sql, warn };
